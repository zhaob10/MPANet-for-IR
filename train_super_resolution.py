...
opt = Config('training_resolution.yml')    #实例化这些训练参数
...
import utils
from dataloaders.data_rgb import get_training_data_sr2, get_training_data_sr3, get_training_data_sr4, get_validation_data_sr2, get_validation_data_sr3, get_validation_data_sr4
...
if __name__ == '__main__':
    ...
    start_epoch = 1
    mode = opt.MODEL.MODE   #'Super_Resolution'
    session = opt.MODEL.SESSION     #'SIPNet'
    ...
    train_dir = opt.TRAINING.TRAIN_DIR      #'../RealSR_patches/train'
    val_dir   = opt.TRAINING.VAL_DIR        #'../RealSR_patches/val' 
    ...
    train_dataset = get_training_data_sr3(train_dir, img_options_train)
    ...
    # val_dataset_sr2 = get_validation_data_sr2(val_dir)
    # val_loader_sr2 = DataLoader(dataset=val_dataset_sr2, batch_size=8, shuffle=False, num_workers=8, drop_last=False)

    # val_dataset_sr3 = get_validation_data_sr3(val_dir)
    # val_loader_sr3 = DataLoader(dataset=val_dataset_sr3, batch_size=8, shuffle=False, num_workers=8, drop_last=False)

    val_dataset_sr4 = get_validation_data_sr4(val_dir)
    val_loader_sr4 = DataLoader(dataset=val_dataset_sr4, batch_size=8, shuffle=False, num_workers=8, drop_last=False)
        ...
        for i, data in enumerate(tqdm(train_loader), 0):    
            ...
            if i%eval_now==0 and i>0:   #i代表当前的iteration
                ...
                with torch.no_grad():       #这里的目的是在执行np_grad包含的语句时网络停止梯度更新，节省显存，便于测试更大的batch
                    ...
                    # psnr_val_sr2 = []       #一个列表，将验证集中所有batch内计算出的PSNR汇总
                    # for ii, data_val in enumerate((val_loader_sr2), 0):
                    ...
                    #     psnr_val_sr2.append(utils.batch_PSNR(restored, target, 1.)) #计算出一个batch内的PSNR

                    # psnr_val_sr2 = sum(psnr_val_sr2)/len(psnr_val_sr2)  #计算出一个epoch内的psnr
                    
                    # if psnr_val_sr2 > best_psnr_sr2:
                    #     best_psnr_sr2 = psnr_val_sr2
                    #     best_epoch_sr2 = epoch
                    #     best_iter_sr2 = i 
                    #     torch.save({'epoch': epoch, 
                    #                 'state_dict': model_restoration.state_dict(),
                    #                 'optimizer' : optimizer.state_dict()
                    #                 }, os.path.join(model_dir,"model_best_sr2.pth"))

                    # print("[Ep %d it %d\t PSNR ReSR_2: %.4f\t] ----  [best_Ep_ReSR_2 %d best_it_ReSR_2 %d Best_PSNR_ReSR_2 %.4f] " % (epoch, i, psnr_val_sr2,best_epoch_sr2,best_iter_sr2,best_psnr_sr2))

                    ################################RealSR_3#################################

                    # psnr_val_sr3 = []       #一个列表，将验证集中所有batch内计算出的PSNR汇总
                    # for ii, data_val in enumerate((val_loader_sr3), 0):
                    #     target = data_val[0].cuda()     #b,c,h,w  gt-image
                    #     input_ = data_val[1].cuda()     #b,c,h,w  noise-image
                    #     filenames = data_val[2]         #b,str
                    ...
                    #     restored = model_restoration(input_)    #待验证的噪声图像送入网络中
                    #     restored = torch.clamp(restored,0,1) 
                    #     psnr_val_sr3.append(utils.batch_PSNR(restored, target, 1.)) #计算出一个batch内的PSNR

                    # psnr_val_sr3 = sum(psnr_val_sr3)/len(psnr_val_sr3)  #计算出一个epoch内的psnr
                    
                    # if psnr_val_sr3 > best_psnr_sr3:
                    #     best_psnr_sr3 = psnr_val_sr3
                    #     best_epoch_sr3 = epoch
                    #     best_iter_sr3 = i 
                    #     torch.save({'epoch': epoch, 
                    #                 'state_dict': model_restoration.state_dict(),
                    #                 'optimizer' : optimizer.state_dict()
                    #                 }, os.path.join(model_dir,"model_best_sr3.pth"))

                    # print("[Ep %d it %d\t PSNR ReSR_3: %.4f\t] ----  [best_Ep_ReSR_3 %d best_it_ReSR_3 %d Best_PSNR_ReSR_3 %.4f] " % (epoch, i, psnr_val_sr3,best_epoch_sr3,best_iter_sr3,best_psnr_sr3))

                    ################################RealSR_4#################################

                    psnr_val_sr4 = []       #一个列表，将验证集中所有batch内计算出的PSNR汇总
                    for ii, data_val in enumerate((val_loader_sr4), 0):
                        target = data_val[0].cuda()     #b,c,h,w  gt-image
                        input_ = data_val[1].cuda()     #b,c,h,w  noise-image
                        filenames = data_val[2]         #b,str
                        ...
                        restored = model_restoration(input_)    #待验证的噪声图像送入网络中
                        restored = torch.clamp(restored,0,1) 
                        psnr_val_sr4.append(utils.batch_PSNR(restored, target, 1.)) #计算出一个batch内的PSNR

                    psnr_val_sr4 = sum(psnr_val_sr4)/len(psnr_val_sr4)  #计算出一个epoch内的psnr
                    
                    if psnr_val_sr4 > best_psnr_sr4:
                        best_psnr_sr4 = psnr_val_sr4
                        best_epoch_sr4 = epoch
                        best_iter_sr4 = i 
                        torch.save({'epoch': epoch, 
                                    'state_dict': model_restoration.state_dict(),
                                    'optimizer' : optimizer.state_dict()
                                    }, os.path.join(model_dir,"model_best_sr4.pth"))

                    print("[Ep %d it %d\t PSNR ReSR_4: %.4f\t] ----  [best_Ep_ReSR_4 %d best_it_ReSR_4 %d Best_PSNR_ReSR_4 %.4f] " % (epoch, i, psnr_val_sr4,best_epoch_sr4,best_iter_sr4,best_psnr_sr4))
